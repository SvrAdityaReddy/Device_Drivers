...
void my_timer_callback(unsigned long data)
{
	...
	static int p_s = 0;
	int gpio46 = gpio_get_value(hdmi_gpio);
	if(gpio46 == 0 && p_s == 0)
	{
		p_s = 1;
		printk(KERN_INFO "HDMI device connected\n");
	}
	else if (gpio46 == 1 && p_s == 1)
	{
		error = rpi_firmware_property(fw,RPI_FIRMWARE_GET_EDID_BLOCK,&edid_tag1,sizeof(edid_tag1));
		if(edid_tag1.edid_block[96] == 'p')
		{
			printk(KERN_INFO "disconnected HDMI device is projector\n");
			struct device *dev = hdmi_dev.this_device;
			error = kobject_uevent_env(&dev->kobj,KOBJ_CHANGE,envp);
			if (error)
			{
				...
				printk(KERN_INFO "uevent generated\n");
			p_s = 0;
		}
	}
